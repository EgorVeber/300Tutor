...
class SolutionViewModel @Inject constructor(
    private val solutionBundleModel: SolutionBundleModel,
    ...
    init {
        val isGenerateTestByDirectory =
            solutionBundleModel.directoryTestId.isNotEmpty() && solutionBundleModel.workSpaceId.isNotEmpty()
        val isStartTest = solutionBundleModel.testId.isNotEmpty()
        val isGetSolution = solutionBundleModel.solutionId.isNotEmpty()
        viewModelScope.launchJob(tryBlock = {
            loadingState.update { true }
            val testSolutionModel = when {
                isGenerateTestByDirectory -> {
                    startTestDirectoryUseCase.invoke(
                        workSpaceId = solutionBundleModel.workSpaceId,
                        directoryId = solutionBundleModel.directoryTestId,
                        htmlPageTestType = solutionBundleModel.htmlPageTestType
                    )
                }

                isStartTest -> startTestUseCase.invoke(solutionBundleModel.testId)
                isGetSolution -> getSolutionUseCase.invoke(solutionBundleModel.solutionId)
    ...
    fun onSelectRightAnswerCheckButtonClicked(selectRightAnswerCheckButtonUiItem: SelectRightAnswerCheckButtonUiItem) {
        viewModelScope.launchJob(tryBlock = {
            loadingState.update { true }
            ...
            uiItemsState.update { uiItems ->
                uiItems.map { uiItem ->
                    if (uiItem is AnswerSelectRightUiModel && uiItem.questionId == currentQuestionId) {
    ...
    fun onCheckedChangeSelectRightAnswer(questionId: String, answerText: String, checked: Boolean) {
        ...
        uiItemsState.update { uiItems ->
            uiItems.map { uiItem ->
                if (uiItem is AnswerSelectRightUiModel && uiItem.questionId == questionId && uiItem.answer == answerText) {
    ...
    fun onDetailedAnswerClicked(
        ...
        viewModelScope.launchJob(tryBlock = {
            ...
            Collections.replaceAll(
                ...
                /* newVal = */ DetailedAnswerYourAnswerUiItem(answer)
            )
    ...
    fun onDetailedAnswerTextChanged(item: DetailedAnswerInputUiItem, text: String) {
        uiItemsState.update { uiItems ->
            uiItems.map { uiItem ->
                if (uiItem is DetailedAnswerInputUiItem && uiItem.questionId == item.questionId) {
                    uiItem.copy(inputAnswer = text)
                } else {
                    uiItem
                }
            }
